"""Retail domain tools. Ported 1:1 from tau2-bench (tool implementations identical)."""
from __future__ import annotations

import json
from typing import Callable, List

from benchmarks.tau2.retail.data_model import (
    GiftCard,
    Order,
    OrderPayment,
    PaymentMethod,
...
class RetailTools:
    ...
    def get_tool_callables(self) -> list[Callable]:
        """Return bound methods suitable for MiniAgent.register_tools()."""
        return [
            self.calculate,
            self.cancel_pending_order,
            self.exchange_delivered_order_items,
            self.find_user_id_by_name_zip,
            self.find_user_id_by_email,
            self.get_order_details,
            self.get_product_details,
            self.get_item_details,
            self.get_user_details,
            self.list_all_product_types,
            self.modify_pending_order_address,
            self.modify_pending_order_items,
            self.modify_pending_order_payment,
            self.modify_user_address,
            self.return_delivered_order_items,
